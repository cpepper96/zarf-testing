...
func addInstallFlags(flags *flag.FlagSet) {
	...
	flags.Bool("upgrade", false, heredoc.Doc(`
		Whether to test an in-place upgrade of each package from its previous revision if the
	...
	flags.String("release-name", "", heredoc.Doc(`
		Name for the release. If not specified, is set to the package name and a random 
		identifier.`))
